...
**投稿後にfrontmatterが壊れる問題を修正**

投稿時に `status` / `note_url` / `publish_date` / `publish_error` を書き戻す処理が、
frontmatter全体をパースして作り直していたため、次のものが毎回失われていました。

| 対象 | 投稿後 |
| --- | --- |
| `tags` / `aliases` などのリスト | 全項目が消え `tags: ""` になる |
| `description: \|` などの複数行の値 | 中身が消え、YAMLとして不正になる |
| frontmatter内のコメント行 | 消える |
| `related: "[[別のノート]]"` | クォートが外れ配列として解釈される |
| ネストしたキー | トップレベルへ引き上げられる |
| `---` の直後の空行 | 消える |

v1.2.16では**更新するキーの行だけを差し替える**方式に変更し、
それ以外の行には一切触れないようにしました。

- 値には必要に応じてYAMLのクォートを付ける（`:` や改行を含むエラーメッセージも安全）
- タイトルに `$&` などが含まれていても壊れない（置換パターンとして解釈されていた）
- frontmatter内のコメント行（`# メモ`）を記事タイトルのH1と誤認しないよう修正
